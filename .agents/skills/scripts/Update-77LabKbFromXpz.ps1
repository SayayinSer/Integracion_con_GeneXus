...
`Rebuild-77LabKbIntelligenceIndex.ps1` na mesma pasta deste wrapper.
...
.EXAMPLE
.\Update-77LabKbFromXpz.ps1 -InputPath C:\Exports\MeuPacote.xpz -ExpectedItems 'Transaction:Cliente'

.EXAMPLE
.\Update-77LabKbFromXpz.ps1 -InputPath C:\Exports\MeuPacote.xpz -ExpectedItems 'Transaction:Cliente', 'Procedure:GeraBoleto'
#>
...
$enginePath = Join-Path $SharedSkillsRoot "scripts-xpz\Sync-GeneXusXpzToXml.ps1"
...
if (-not $IndexUpdateScriptPath) {
    $IndexUpdateScriptPath = Join-Path $PSScriptRoot "Rebuild-77LabKbIntelligenceIndex.ps1"
