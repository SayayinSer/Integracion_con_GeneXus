...
Caminho opcional para `Test-77LabKbGate.ps1`.

.PARAMETER MetadataWrapperTestPath
Caminho opcional para `Test-77LabKbMetadataWrapper.ps1`.
...
.EXAMPLE
.\Test-77LabKbSetupAudit.ps1

.EXAMPLE
.\Test-77LabKbSetupAudit.ps1 -KbRoot C:\KB
#>
...
if (-not $GateWrapperPath) {
    $GateWrapperPath = Join-Path $PSScriptRoot 'Test-77LabKbGate.ps1'
}

if (-not $MetadataWrapperTestPath) {
    $MetadataWrapperTestPath = Join-Path $PSScriptRoot 'Test-77LabKbMetadataWrapper.ps1'
}
...
$enginePath = Join-Path $SharedSkillsRoot 'scripts-xpz\Test-XpzSetupAudit.ps1'
